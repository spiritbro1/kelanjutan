...
export class PythConnection {
  ...
  private handlePriceAccount(key: PublicKey, account: AccountInfo<Buffer>, slot: number) {
    ...
    const kabeh:any={}
    kabeh[this.priceAccountKeyToProductAccountKey[key.toString()]]=priceData;
    console.log(kabeh)
...
// const productAccountKeyToProduct: Record<string, Product> = {}
// const priceAccountKeyToProductAccountKey: Record<string, string> = {}
...
//   }
// }
// function handleAccount(key: PublicKey, account: AccountInfo<Buffer>, productOnly: boolean, slot: number) {
//   const base = parseBaseData(account.data)
//   // The pyth program owns accounts that don't contain pyth data, which we can safely ignore.
//   if (base) {
//     switch (base.type) {
...
//   }
// }
// export async function getAll(connection: Connection, pythProgramKey: PublicKey, commitment: Commitment = 'finalized'){
//   const accounts = await connection.getProgramAccounts(pythProgramKey, commitment)
//   const currentSlot = await connection.getSlot(commitment)
//   for (const account of accounts) {
//     handleAccount(account.pubkey, account.account, true, currentSlot)
//   }
// }